...
def getRank(request):
    ...
    for i in set(location):
        ...
        avgSort[int(i)-1]=avercount*(-1)

    context=[]
    # c = sorted(range(len(avgSort)), key=lambda k: avgSort[k])
    avgSort=np.array(avgSort)
    c=np.argsort(avgSort).tolist()
    print(avgSort)
    print(c)
    for i in set(location):
        row = [0,0,0]
...
def download(request):
    with open('/root/PoliceCount.apk','rb') as f:
        apk=f.read()
    response = HttpResponse(apk)
    response['Content-Type'] = 'application/vnd.android.package-archive'
    response['Content-Disposition'] = 'attachment;filename="PoliceCount.apk"'
    return response
